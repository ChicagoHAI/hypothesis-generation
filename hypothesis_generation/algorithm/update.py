    ...
    def update(
        ...
        for i in range(start_sample, num_train_examples):
            if self.num_wrong_scale > 0:
                num_wrong_to_add_bank = self.k * i / num_train_examples * self.num_wrong_scale

            current_example = i+1
            print(f"Training on example {i}")

            top_k_hypotheses = sorted(hypotheses_bank, key=lambda x: hypotheses_bank[x].reward, reverse=True)[:self.k]
            
            # check if the hypothesis works for the generated hypotheses
            num_wrong_hypotheses = 0
            for hypothesis in top_k_hypotheses:
                pred, label = self.inference_class.predict(self.train_data, i, {hypothesis: hypotheses_bank[hypothesis]}, self.use_system_prompt)
                if pred != label:
                    num_wrong_hypotheses += 1
                    hypotheses_bank[hypothesis].update_info_if_not_useful(current_example, self.alpha)
                else:
                    hypotheses_bank[hypothesis].update_info_if_useful(current_example, self.alpha)
            ...
            if num_wrong_hypotheses >= num_wrong_to_add_bank or len(top_k_hypotheses) == 0:
                ...
                if len(wrong_example_ids) == self.update_batch_size*self.num_hypotheses_to_update:
                    new_hyp_bank = {}
                    
                    # generate new hypotheses
                    for j in range(self.num_hypotheses_to_update):
                        new_hypotheses = self.generation_class.batched_hypothesis_generation(wrong_example_ids, current_example, self.update_hypotheses_per_batch)
            ...
            if num_wrong_hypotheses >= num_wrong_to_add_bank or len(top_k_hypotheses) == 0:
                ...
                if len(wrong_example_ids) == self.update_batch_size*self.num_hypotheses_to_update:
                    ...
                    for j in range(self.num_hypotheses_to_update):
                        ...
                        new_hypotheses = self.balance_by_sample(new_hypotheses, current_example, hypotheses_bank[max_visited].num_visits, self.num_init, self.alpha, self.use_system_prompt)
                        if self.only_best_hypothesis:
                            best_hypothesis = max(new_hypotheses, key=lambda x: new_hypotheses[x].reward)
                            new_hyp_bank.update({best_hypothesis: new_hypotheses[best_hypothesis]})
                        else:
