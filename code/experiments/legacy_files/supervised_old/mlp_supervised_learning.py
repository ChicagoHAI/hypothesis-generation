...
def main():
    ...
    # only update the final projection layer
    # optimizer = torch.optim.Adam(model.theta.parameters(), lr=learning_rate)

    # update all layers
    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)

    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1000000, gamma=1.0)

    training_args = TrainingArguments(
        output_dir='./supervised_mlp',
