...
class MultiHypHierarchicalInference(DefaultInference):
    ...
    def _parse_stump_response(response):
        ...
        # Parse the response to extract groups and their conditions/hypotheses
        groups = {}
        current_group = None
        ...
        for line in lines:
            ...
            # Look for group indicators (e.g., "Group 1:", "1.", etc.)
            ...
            if group_match:
                ...
                # This is a hypothesis under the current group
                ...
                # This is a numbered hypothesis under the current group
    ...
    def _create_stump_from_hypotheses(
            self,
            hyp_bank,
            cache_seed=None,
            **generate_kwargs,
        ...
        prompt_input = self.prompt_class.create_stump_from_hypotheses(hyp_bank)
    ...
    def _determine_sample_groups_batched(
            self,
            data: pd.DataFrame,
            groups,
            cache_seed=None,
        ...
        for response in responses:
            ...
            try:
                ...
                # Default to group 1 if parsing fails
                print(f"Failed to parse group number from response: {response}, defaulting to group 1")
                group_nums.append(1)
    ...
    def _stump_batched_predict(
        ...
        group_nums = self._determine_sample_groups_batched(
            data, groups, cache_seed, max_concurrent, **generate_kwargs
        )

        # Create prompts for all samples with their respective group hypotheses
        prompt_inputs = []
        sample_indices = []
        ...
        for idx in range(len(data)):
            ...
            if group_num in groups:
                ...
                prompt_input = self.prompt_class.hierarchical_inference(
                    hyp_dict, data, idx, group_condition
                )
                prompt_inputs.append(prompt_input)
                sample_indices.append(idx)
            else:
                raise Exception(f"Group {group_num} not found in groups.")
    ...
    def run_inference_final(
        ...
        # Create stump (groups) from hypotheses
        groups = self._create_stump_from_hypotheses(
            hyp_bank,
            cache_seed=cache_seed,
            **generate_kwargs,
        )
